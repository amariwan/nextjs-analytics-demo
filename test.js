/*!
 *
 * Copyright (c) 2020-2021 Witt solutions (witt-solutions.de)
 * Auth Aland Mariwan (aland-mariwan.de)
...
(function (m, q) {
    ...
    var q = window,
        ...
        N = function (T) {
            ...
            T.data && T.data.length && (V = H("tbody", {
    ...
    P.prototype.swap = function (T) {
        ...
        var V, U = this,
            W = document.createElement("th", {
                class: "text-gray-700 dark:text-gray-400px-4 py-3 text-sm"
            });
    ...
    Q.prototype.build = function (T) {
        ...
        return W.length || (W = T.map(function () {
            ...
            }), T[Y] || T[Y].length || (T[Y] = ""), U.innerHTML = T[Y], U.data = T[Y], V.appendChild(U)
        }), V
    ...
    return S.init = function (T) {
        if (this.initialized || K.contains(this.table, "dataTable-table w-full whitespace-no-wrap")) return !1;
        var U = this;
        ...
        if (U.body = U.table.tBodies[0], U.head = U.table.tHead, U.foot = U.table.tFoot, U.body || (U.body = H("tbody", {
                class: "bg-white divide-y dark:divide-gray-700 dark:bg-gray-800"
            }), U.table.appendChild(U.body)), U.hasRows = 0 < U.body.rows.length, !U.head) {
            var ba = H("thead"),
                ca = H("tr", {
        ...
        if (U.headings = [], U.hasHeadings = 0 < U.head.rows.length, U.hasHeadings && (U.header = U.head.rows[0], U.headings = [].slice.call(U.header.cells)), !V.header && U.head && U.table.removeChild(U.table.tHead), V.footer ? U.head && !U.foot && (U.foot = H("tfoot", {
            ...
            var ea = H("select", {
                class: "dataTable-selector w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray"
            });
        ...
        if (V.searchable) {
            var fa = "<div class='dataTable-search'><input class='w-full pl-8 pr-2 text-sm text-gray-700 placeholder-gray-600 bg-gray-100 border-0 rounded-md dark:placeholder-gray-500 dark:focus:shadow-outline-gray dark:focus:placeholder-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:placeholder-gray-500 focus:bg-white focus:border-purple-300 focus:outline-none focus:shadow-outline-purple form-input' placeholder='" + V.labels.placeholder + "' type='text'></div>";
        ...
        T.labels = [], T.headings && T.headings.length && F(T.headings, function (U, V) {
            if (T.labels[V] = U.textContent, K.contains(U.firstElementChild, "px-4 py-3") && (U.innerHTML = U.firstElementChild.innerHTML), U.sortable = "false" !== U.getAttribute("px-4 py-3"), U.originalCellIndex = V, T.options.sortable && U.sortable) {
                var W = H("a", {
                    href: "#",
                    class: "dataTable-sorter text-white",
                    html: U.innerHTML
                });
        ...
        U.searchable && (T.input = T.wrapper.querySelector(".form-input"), T.input && G(T.input, "keyup", function () {
            T.search(this.value)
        })), G(T.wrapper, "click", function (W) {
            var X = W.target;
            "a" === X.nodeName.toLowerCase() && (X.hasAttribute("data-page") ? (T.page(X.getAttribute("data-page")), W.preventDefault()) : U.sortable && K.contains(X, "dataTable-sorter text-white") && "false" != X.parentNode.getAttribute("px-4 py-3") && (T.columns().sort(T.activeHeadings.indexOf(X.parentNode) + 1), W.preventDefault()))
        })
        ...
        this.table.innerHTML = this.initialLayout, K.remove(this.table, "dataTable-table w-full whitespace-no-wrap"), this.wrapper.parentNode.replaceChild(this.table, this.wrapper), this.initialized = !1
    }, S.update = function () {
