...
(function(m, q) {
    ...
    var q = window,
        ...
        A = {
            ...
            labels: {
                ...
                info: "Zeigen {start} bis {end} von {rows} Einträgen"
            },
            layout: {
                top: "{select}{search}",
                bottom: "{info}{pager}"
    ...
    return S.init = function(T) {
        ...
        T.labels = [], T.headings && T.headings.length && F(T.headings, function(U, V) {
            if (T.labels[V] = U.textContent, K.contains(U.firstElementChild, "dataTable-sorter px-4 py-3") && (U.innerHTML = U.firstElementChild.innerHTML), U.sortable = "false" !== U.getAttribute("data-sortable"), U.originalCellIndex = V, T.options.sortable && U.sortable) {
                var W = H("a", {
                    href: "#",
